...
router.post("/signup", signUpVerify, (req, res) => {
    ...
    User.findByAUIID(req.body.auiID)
        .then(user => {
            if (user)
                throw new ServiceError(ERROR_VALUE.account_auiID_exists);
        ...
        // var code = QRCode(req.body.email,{
        //     size: 250
        // });
        // Create new user account in database
        return User.createNew(req.body.firstName, req.body.lastName, req.body.auiID, req.body.email, req.body.password, req.body.phoneNumber,
             req.body.country, req.body.gender, req.body.dateOfBirth);
        ...
        .then(activationToken => {
           ...
           Mailer.sendActivationMail(req.body.email, activationToken.payload);
...
router.post("/signin", signInVerify, (req, res) => {
    ...
    User.findByCredentials(req.body.identity, req.body.password)
        ...
        .then(token => {
            ...
            //     result.imageUrl = req.user.imageUrl;
            //
            ...
            // Check if user wallet has been created
            // if (req.user.wallet_id) {
            //     result.isWalletCreated = true;
            //     result.walletAddress = req.user.wallet_id.address;
            // } else {
            //     result.isWalletCreated = false;
            // }
 ...
 */
router.put("/password-recovery/password", changePasswordRecoveryVerify, recoveryTokenVerify, (req, res) => {
    Promise.resolve()
        .then(() => {
            // Check if recovery code matches
            if (req.recoveryToken.payload !== req.body.recoveryCode)
        ...
        .then(() => {
            // Delete existing recovery token
            return UserToken.delete(AccessType.RECOVERY, req.user._id);
        })
        .then(() => {
